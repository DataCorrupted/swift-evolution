...
* Authors: [Peter Rong](https://github.com/DataCorrupted)
* Review Manager: TBD
...
Add `@objcDirect` attribute to functions to reduce the amount of unncessary metadata.

## Motivation

Binary size is a key concern for mobile applications. Large apps are less likely to be downloaded, and generally have a slower start up time.
One practical way to reduce binary size is through elimination of unncessary metadata.
Objective C introduced `@objc_direct` so functions can be statically dispatched without the need to generate a big trunk of runtime metadata [[1](https://reviews.llvm.org/D69991)].
In order to further reduce runtime metadata on swift size, developers should have the ability to manually mark some functions as `@objcDirect` as well to match the behavior on the Objective C side.

## Proposed solution

We should introduce a new attribute `@objcDirect` that works as following:

```swift
class Foo : NSObject {
    @objc @objcDirect
    public func foo0() -> Int{
        return 0;
    }
    func foo1() {}
}
extension FooExt {
    @objc @objcDirect
    func fooExtension() -> Int { return 0;}
}
```
...
The design and implementation of `@objcDirect` should take small incremental steps.
This is due to the fact that `@objcDirect` can interact with many other attributes, including but not limited to `@objc(name)`, [`@cdecl`](https://forums.swift.org/t/formalizing-cdecl/40677), [`@implementation`](), etc.

As a first step, we put the following restrictions on `@objcDirect`:

1. Only methods in the classes inherited from `NSObjct` can apply `@objcDirect`, this includes `extension`s.
2. `@objcDirect` must be applied to each individual functions explicitly. To mark the entire `class`/`extension` altogether we should introduce `@objcDirectMember` to match `@objcMember` in the future.
3. `@objcDirect` must be applied when `@objc` exists, and `@objc` can't take arguments.

The idea is that we can make sure that this feature works before we roll out more advanced features. Here's a list of future todos:

1. `@objcDirect` on functions in `protocol`s.
3. `@objcDirect` with `objc(name)`, this requires us to change the function name exposed to ObjC
2. `@objcDirectMember` on `class`, `extension`, and `protocol`.
...
### ClangPrinter
If a function is annotated with `@objcDirect`, the bridging header will include an attribute `__attribute__((objc_direct))` to inform the Objective C compiler that this method is statically dispatched.
